...
<?php
	...
	if($_POST['fl']=='title'){
		$JCSQL_title=file('../TTSQL/Admin/Plug/Zhanqun/title.txt');
		$count=count($JCSQL_title);
		$rand=rand(0, $count-1);
		echo $JCSQL_title[$rand];
	}
	if($_POST['fl']=='keywords'){
		$keywords=file('../TTSQL/Admin/Plug/Zhanqun/keywords.txt');
	...
	if($_POST['fl']=='description'){
		$description=file('../TTSQL/Admin/Plug/Zhanqun/description.txt');
