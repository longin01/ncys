	...
	function get_http_code($url) {  
		...
		function GETZIP($ERR1,$ERR2) { 
			...
			while (!feof($VODZIPCONTER)) {
				fwrite($VODZIPCONTERURL, fread($VODZIPCONTER,10400));
			}
				fclose($VODZIPCONTER);
		...
		if($httpCode =='200'){
			...
			echo '更新服务器通讯失败，极有可能正在维护中或者其他原因，请联系www.TTCMS.net官方咨询，本日跳过数据更新进入站点';
			file_put_contents('../TTSQL.TTSQL',date('Ymd', time()));
