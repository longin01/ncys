...
<div class="v-read">
	...
	<!-- 影片介绍 -->
	<div class="read-info">
		<div class="read-img"> <?php echo $this->value['overview']; ?>......</div>
		<!-- 视频标题 -->
		...
		<div class="read-text uk-margin-top-remove uk-clearfix">
			<p class="uk-float-left">小说分类:<span><?php echo $this->value['ListName']; ?></span></p><p class="uk-float-left">更新时间:<span><?php echo $this->value['Time']; ?></span></p>
		</div>	
		<div class="read-btn uk-margin-top uk-margin-bottom uk-clearfix">
			<a href="<?php echo $this->value['CUrl']; ?>" class="uk-float-left">点击阅读</a><a href="<?php echo $this->value['CUrl']; ?>" class="uk-float-right">快速阅读</a>
		</div>
	</div>
</div>
...
<div class="v-type-favorite uk-clearfix">
	<!-- 相关视频 标题 -->
	<div class="v-type-title uk-clearfix uk-margin-top uk-margin-small-bottom"><h3 class="uk-float-left">推荐小说</h3></div>
	<!-- 视频缩略图 -->
	<ul class="recom recom-play">
		<?php  $BookType=$this->value['BookType']; $MYSQLVOD=(array)$this->value['BookTypeNCSQL'];shuffle($MYSQLVOD);  $Count=4;for ($x=0; $x<=$Count-1; $x++) {  $MYSQLVODs=$MYSQLVOD[$x];  ?>
			<li>
				<a href="<?php 	echo  $Host1.'book_detail'.$Host2.$MYSQLVODs['a_id'].$Host3.$BookType.$Host4; ?>" class="uk-display-block ">
					<div class="re-pic uk-float-left"><img  class="v-cover lazy" data-original="<?php 	echo  $MYSQLVODs['a_pic'] ?>" alt="<?php 	echo  $MYSQLVODs['a_name'] ?>"></div>
					<div class="re-info">
						...
						<div class="re-detail">
							...
							<span><i class="icon iconfont icon-zan"></i>畅读小说乐趣无限</span>
						</div>
					</div>
				</a>
			</li>
