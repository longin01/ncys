...
<div class="v-type " style="margin-top: 0rem; ">
    <div class="v-type-title uk-clearfix"><div><h3 class="type-h3">小说<?php  echo $this->value['BookTypeName']; ?>列表</h3></div></div>
            <ul class="v-list uk-clearfix uk-display-block">
			<?php  $MYSQLVODS=(array)$this->value['BookTypeNCSQL'];$BookTypeId=$this->value['BookTypeId'];$GetMb_page=$this->value['BookTypePage']; $Count=8;$MYSQLVODS=PAGE($MYSQLVODS,$GetMb_page,$Count);  foreach($MYSQLVODS['source'] as $s){  ?>
                <li class="v-item v-item">
                    <a href="<?php 	echo  $Host1.'book_detail'.$Host2.$s['a_id'].$Host3.$BookTypeId.$Host4; ?>" class="uk-display-block" style="position:relative">
                        <div class="v-cover lazy" data-original="<?php 	echo  $s['a_pic'] ?>"></div>
