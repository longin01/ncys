			...
			<?php $mubantou=$this->value['Header'];$this->show($mubantou); ?>
				<div class="right-container">
					...
					<div class="list-videos uk-margin-top uk-margin-bottom">
						<div class="video-head uk-clearfix"><h3 class="h3-color uk-float-left">套图<?php  echo $this->value['PicTypeName']; ?></h3></div>
                    <div class="video-content uk-grid uk-grid-small">
						<?php  $MYSQLVODS=(array)$this->value['PicTypeNCSQL'];$PicTypeId=$this->value['PicTypeId'];$GetMb_page=$this->value['PicTypePage']; $Count=9;$MYSQLVODS=PAGE($MYSQLVODS,$GetMb_page,$Count);  foreach($MYSQLVODS['source'] as $s){  ?>	
								<div class="uk-width-1-3 uk-margin-bottom">
								<a href="<?php 	echo  $Host1.'pic_detail'.$Host2.$s['a_id'].$Host3.$PicTypeId.$Host4; ?>" class="video-pic uk-display-block">
									<span class="live-tag"></span>
									<img src="<?php 	echo  $s['a_remarks'] ?>" alt="<?php 	echo  $s['a_name'] ?>">
								</a>
								<div class="video-title">
									<a href="#" class="uk-margin-small-bottom uk-margin-small-top uk-display-block"><?php 	echo  $s['a_name'] ?></a>
